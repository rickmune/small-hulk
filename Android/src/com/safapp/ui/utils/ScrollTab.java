...
public class ScrollTab  extends ViewGroup implements OnPageChangeListener{
	...
	protected OnSwipeTabs onSwipeTabs;
	protected final String TAG = "ScrollTab";

	private ScrollTabsAdapter mAdapter;
	...
	private int mTextColor = 0xff57C8F2;//0xff949494;
	...
	public ScrollTab(Context context) {
		this(context, null);
		Log.d(TAG, "ScrollTab(Context context)");
	}

	public ScrollTab(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		Log.d(TAG, "ScrollTab(Context context, AttributeSet attrs): "+attrs.getAttributeCount()+" : "+attrs.toString());
	}

	public ScrollTab(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		Log.d(TAG, "ScrollTab(Context context, AttributeSet attrs, int defStyle): "+defStyle);
		final TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.ScrollTabs, defStyle, 0);

		mBottomBarColor = a.getColor(R.styleable.ScrollTabs_bottomBarColor,
				mBottomBarColor);
		mBottomBarHeight = a.getDimensionPixelSize(
				R.styleable.ScrollTabs_bottomBarHeight, 2);
		mTabIndicatorHeight = a.getDimensionPixelSize(
				R.styleable.ScrollTabs_tabIndicatorHeight, 3);

		a.recycle();

		init();
		Log.d(TAG, "is implementation Done");
		try {
			onSwipeTabs = (OnSwipeTabs)context;
		} catch (Exception e) {
		}
	}
	 ...
	 * @param adapter
	 */
	public void setAdapter(ScrollTabsAdapter adapter) {
		if (mAdapter != null) {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < count; i++) {
			...
			v.layout(mCurrentTabPos[i], this.getPaddingTop(), mCurrentTabPos[i]
					+ v.getMeasuredWidth(), this.getPaddingTop() + v.getMeasuredHeight());
		}
	}
	...
	protected void dispatchDraw(Canvas canvas) {
		if (mCurrentPos != -1) {
			...
			for (int i = 0; i < count; i++) {
				final TextView tab = (TextView) getChildAt(i);
				if (mCurrentPos == i) {
					tab.setTextColor(interpolateColor(/*mBottomBarColor*/mTextColor,

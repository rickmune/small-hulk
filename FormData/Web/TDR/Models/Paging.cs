...
{
    public class PagedList2<T> : List<T>, IPagedList<T>
    {
        public PagedList2(IEnumerable<T> source, int index, int pageSize, int? totalCount = null)
            : this(source.AsQueryable(), index, pageSize, totalCount)
        {
        }

        public PagedList2(IQueryable<T> source, int index, int pageSize, int? totalCount = null)
        {
            if (index < 0)
